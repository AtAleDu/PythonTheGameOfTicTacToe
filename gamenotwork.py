...
class MiniGrid:
    ...
    def create_grid(self):
        for row in range(3):
            for col in range(3):
                button = Button(self.master, text=' ', width=2, height=1,  # Уменьшили размер кнопки
    ...
    def highlight_grid(self, highlight):
        color = 'yellow' if highlight else 'SystemButtonFace'
        for row in self.buttons:
            for button in row:
                button['background'] = color
...
def computer_turn():
    ...
    if game_run and current_mini_grid and not current_mini_grid.winner:
        ...
        if empty_cells:
            ...
            current_mini_grid.make_move(row, col, current_player)
            if check_big_field_win():
                game_run = False
                show_winner_message(current_player)
            else:
                ...
                current_mini_grid = None
def computer_move():
    global current_mini_grid, game_run
    if game_run and current_mini_grid:
        if current_mini_grid.winner:
            current_mini_grid = None
            return
        ...
        if empty_cells:
            ...
            overall_winner = check_big_field_win()
            if overall_winner:
                game_run = False
                show_winner_message(overall_winner)
            else:
                next_mini_grid = big_field[row][col]
                if next_mini_grid.winner:
                    current_mini_grid = None
                else:
                    current_mini_grid = next_mini_grid
